...
**Domain:** `Finance / Tax` · **Suite:** `Finance Document Suite` · **Accent:** `#67e8f9`
...
**Tax Doc Parser** is a standalone, browser-based AI workflow app for **Finance / Tax**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.

**Core job:** Tax documents → classified and extracted fields.
...
|---|---:|:---:|---|---|
| `tax_documents`<br>Tax documents | text | Yes | — | Affects input: Tax documents. |
...
| `tax_year`<br>tax year | text | No | — | Affects customization: tax year. |
| `entity_type`<br>entity type | text | No | — | Affects customization: entity type. |
| `form_types`<br>form types | text | No | — | Affects customization: form types. |
| `redaction`<br>redaction | text | No | — | Affects customization: redaction. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9158
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9158`. |
...
- Tax documents
- classified and extracted fields
...
- Export download
- Job history
- Country tax database/accounting optional
...
```bash
curl http://127.0.0.1:9158/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9158/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9158 uvicorn app.main:app --host 0.0.0.0 --port 9158
```
...
- `plugins/tax-doc-parser.json` for plugin metadata.
